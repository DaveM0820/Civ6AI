...
local function Civ5AiEnsureSeatStepEvent()
...
local function Civ5AiApplyStepLabel(labelControl, playerID)
	if labelControl == nil then
		return;
	end
	if Civ5Ai_Config == nil or Civ5Ai_Seats == nil or not Civ5Ai_Config.ShouldRunBridge(playerID) then
		labelControl:SetHide(true);
		return;
	end
	local text = Civ5Ai_Seats.StepLabel(playerID);
...
local function Civ5AiScoreboardLeaderName(player, playerID)
	-- LLM seats are SS_TAKEN humans in simultaneous turns. Vanilla DiploList
	-- prefers the MP nick in that case, which is empty or "Player N",
	-- not Suleiman / Montezuma. Always use the leader title.
...
local function Civ5AiOnSeatStepChanged(playerID)
...
Civ5AiEnsureSeatStepEvent();
if LuaEvents ~= nil and LuaEvents.Civ5AiSeatStepChanged ~= nil then
	LuaEvents.Civ5AiSeatStepChanged.Add(Civ5AiOnSeatStepChanged);
end
...
function LeaderSelected( ePlayer )
    ...
    elseif Civ5Ai_Config ~= nil and Civ5Ai_Config.IsManagedSeat ~= nil and Civ5Ai_Config.IsManagedSeat(ePlayer) then
        Events.OpenPlayerDealScreenEvent( ePlayer );
    else
...
function UpdateDisplay()
	...
	Civ5AiApplyStepLabel(Controls.MyCiv5AiStepLabel, g_iPlayer);

	local textBoxSize = Controls.NameBox:GetSizeX() - Controls.LeaderName:GetOffsetX();
	TruncateString(Controls.LeaderName, textBoxSize, Civ5AiScoreboardLeaderName(g_pPlayer, g_iPlayer), "  (" .. Locale.ConvertTextKey( "TXT_KEY_YOU" ) .. ")");
	...
	for iPlayerLoop = 0, GameDefines.MAX_MAJOR_CIVS-1, 1 do
		...
		if (iPlayerLoop ~= g_iPlayer and pOtherPlayer:IsAlive()) then
			...
			if (g_pTeam:IsHasMet(iOtherTeam)) then
				...
				TruncateString(controlTable.LeaderName, textBoxSize, Civ5AiScoreboardLeaderName(pOtherPlayer, iPlayerLoop));
				...
				Civ5AiApplyStepLabel(controlTable.Civ5AiStepLabel, iPlayerLoop);
	...
	--CBP
				table.insert(myScoreEntries, Locale.Lookup("TXT_KEY_DIPLO_MY_SCORE_ALLIES", pOtherPlayer:GetScoreFromMinorAllies()));
				table.insert(myScoreEntries, Locale.Lookup("TXT_KEY_DIPLO_MY_SCORE_MILITARY", pOtherPlayer:GetScoreFromMilitarySize()));
				if (not Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
...
-- FPS: SerialEventGameDataDirty is high-frequency; full scoreboard rebuild
-- every dirty event pegs the game thread with the mod active.
local _civ5ai_diploListLast = 0
local function UpdateDisplayThrottled()
	local now = (os and os.clock and os.clock()) or 0
	if (now - _civ5ai_diploListLast) < 0.25 then
		return
	end
	_civ5ai_diploListLast = now
	UpdateDisplay()
end
Events.SerialEventGameDataDirty.Add(UpdateDisplayThrottled);
