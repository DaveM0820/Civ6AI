...
-- LLM seats are SS_TAKEN humans. Vanilla MPList uses GetNickName() for humans,
-- which is empty in this lobby, so the HUD row is blank.
local function Civ5AiScoreboardLeaderName(pPlayer)
	local playerID = pPlayer:GetID();
	if PreGame ~= nil and PreGame.GetLeaderName ~= nil then
		local pre = PreGame.GetLeaderName(playerID);
		if pre ~= nil and pre ~= "" then
			return Locale.ConvertTextKey(pre);
		end
	end
	if pPlayer.GetLeaderType ~= nil and GameInfo ~= nil and GameInfo.Leaders ~= nil then
		local info = GameInfo.Leaders[pPlayer:GetLeaderType()];
		if info ~= nil and info.Description ~= nil and info.Description ~= "" then
			return Locale.ConvertTextKey(info.Description);
		end
	end
	if pPlayer.GetName ~= nil then
...
function UpdatePlayerData( pPlayer, controlTable )
  ...
  local bMet = g_pLocalTeam:IsHasMet( iTeam );
    ...
    -- name
    -- Local SP stays "You". Unmet stays "?". Met humans with empty MP nick
    -- get the leader title (William / Genghis / Wu / Haile), not a blank row.
    if( bIsHuman ) then
			...
			elseif( not bMet ) then
				...
				if nick ~= nil and nick ~= "" then
					controlTable.Name:SetText(nick);
				else
					controlTable.Name:SetText(Civ5AiScoreboardLeaderName(pPlayer));
				end
			end
