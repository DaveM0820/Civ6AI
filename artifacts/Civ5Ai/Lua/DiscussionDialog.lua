...
-- Civ5Ai: close forced-response discussion dialogs (friendship, denounce, etc.)
-- without accept/refuse so the map stays visible for llm_human seats.
function Civ5Ai_DeferDiploDiscussionUi()
	if g_iAIPlayer == nil or g_iAIPlayer == -1 or OnBack == nil then
		return false
	end
	if ContextPtr ~= nil and ContextPtr.IsHidden ~= nil and ContextPtr:IsHidden() then
		return false
	end
	local host = ExposedMembers ~= nil and ExposedMembers.Civ5Ai or nil
	if host ~= nil and host.QueueDiscussionProposal ~= nil and Game ~= nil then
		host.QueueDiscussionProposal(Game.GetActivePlayer(), g_iAIPlayer, g_DiploUIState, g_iDiploData)
	end
	OnBack(true)
	local host = ExposedMembers ~= nil and ExposedMembers.Civ5Ai or nil
	if host ~= nil and host.OnDiploTradeDeferred ~= nil and Game ~= nil then
		host.OnDiploTradeDeferred(Game.GetActivePlayer())
	end
	if Civ5Ai_Util ~= nil then
		Civ5Ai_Util.Log(
			"diplo|defer_discussion_ui|ai="
				.. tostring(g_iAIPlayer)
				.. "|state="
				.. tostring(g_DiploUIState)
		)
	end
	return true
end

function Civ5Ai_MaybeDeferDiscussionUi()
	local host = ExposedMembers ~= nil and ExposedMembers.Civ5Ai or nil
	if host == nil or host.ShouldDeferDiploTradeUi == nil or Game == nil then
		return
	end
	local active = Game.GetActivePlayer()
	if not host.ShouldDeferDiploTradeUi(active) then
		return
	end
	Civ5Ai_DeferDiploDiscussionUi()
end

if ExposedMembers ~= nil then
	ExposedMembers.Civ5Ai = ExposedMembers.Civ5Ai or {}
	ExposedMembers.Civ5Ai.DeferDiploDiscussionUi = Civ5Ai_DeferDiploDiscussionUi
end
