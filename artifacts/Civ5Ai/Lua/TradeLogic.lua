...
function OnPropose( iType )
    ...
    if( g_bPVPTrade ) then
        ...
        if( iType == PROPOSE_TYPE ) then
			...
			g_Deal:SetFromPlayer(g_iUs);
			g_Deal:SetToPlayer(g_iThem);
        	if Civ5Ai_Diplo ~= nil and Civ5Ai_Diplo.QueueScratchProposal ~= nil and Civ5Ai_Diplo.QueueScratchProposal(g_iUs, g_iThem) then
        	    -- Civ5Ai: managed seat async queue (no UI.DoProposeDeal)
        	else
        	    UI.DoProposeDeal();
        	end
...
-- Civ5Ai: close AI-human trade without accept/refuse. Use OnBack so leader and
-- trade panels close together; the offer stays queued for the LLM wire response.
function Civ5Ai_DeferDiploTradeUi()
	if g_iThem == nil or g_iThem == -1 or OnBack == nil then
		return false
	end
	OnBack()
	...
	if Civ5Ai_Util ~= nil then
		Civ5Ai_Util.Log("diplo|defer_trade_ui|them=" .. tostring(g_iThem))
	end
	return true
end

function Civ5Ai_MaybeDeferTradeUi()
	local host = ExposedMembers ~= nil and ExposedMembers.Civ5Ai or nil
	...
	Civ5Ai_DeferDiploTradeUi()
end

if ExposedMembers ~= nil then
	ExposedMembers.Civ5Ai = ExposedMembers.Civ5Ai or {}
	ExposedMembers.Civ5Ai.DeferDiploTradeUi = Civ5Ai_DeferDiploTradeUi
end
