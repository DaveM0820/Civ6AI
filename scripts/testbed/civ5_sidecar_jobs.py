...
def _run_job(job: dict[str, Any], repo: Path) -> None:
    ...
    script = str(job.get("script") or "sidecar/run_civ5.py")
    args = job.get("args")
...
def process_sidecar_jobs(
    civ5ai_roots: list[Path] | Path,
    session_id: str | None = None,
) -> int:
    """Run pending sidecar_job.json files. Accepts one root or a list of roots."""
    if isinstance(civ5ai_roots, Path):
        roots = [civ5ai_roots]
    else:
        roots = list(civ5ai_roots)
    active = (session_id or "").strip()
    ran = 0
    seen_jobs: set[str] = set()
    for civ5ai_root in roots:
        ...
        for job_path in sorted(sessions.rglob("sidecar_job.json")):
            if active:
                try:
                    job_path.relative_to(sessions / active)
                except ValueError:
                    continue
...
def discover_civ5ai_roots(primary: Path) -> list[Path]:
    """Watch the mod runtime folder the game actually uses."""
    from civ5_host_channel import civ5_apply_pending_dirs
    ...
    roots: list[Path] = []
    seen: set[str] = set()
    for path in [primary, *civ5_apply_pending_dirs()]:
        try:
        ...
        if key in seen:
            continue
        seen.add(key)
        if path.is_dir() or path == primary or key == primary_key:
            roots.append(path)
    if not roots:
