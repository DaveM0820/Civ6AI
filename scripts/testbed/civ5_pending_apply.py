"""Publish LLM apply payloads for Civ5Ai via the hidden InGame inbox."""
from __future__ import annotations

import json
import logging
import re
import sys
import time
from pathlib import Path
from typing import Any

_REPO = Path(__file__).resolve().parents[2]
if str(_REPO) not in sys.path:
    sys.path.insert(0, str(_REPO))

log = logging.getLogger(__name__)

_COMMAND_ID_RE = re.compile(r'"command_id"\s*:\s*"([^"]+)"')
_KIND_RE = re.compile(r'"kind"\s*:\s*"([^"]+)"')


def _kind_from_command_id(command_id: str) -> str | None:
    if command_id.startswith("CMD_research_"):
        return "set_research_tech"
    if command_id.startswith("CMD_prod_"):
        return "queue_production"
    if command_id.startswith("CMD_move_"):
        return "move_unit"
    if command_id.startswith("CMD_found_"):
        return "found_city"
    if command_id.startswith("CMD_fortify_"):
        return "unit_posture_fortify"
    if command_id.startswith("CMD_alert_"):
        return "unit_posture_alert"
    if command_id.startswith("CMD_skip_"):
        return "unit_skip"
    if command_id.startswith("CMD_sleep_"):
        return "unit_posture_sleep"
    if command_id.startswith("CMD_attack_"):
        return "attack_target"
    return None


def salvage_apply_commands(text: str) -> list[dict[str, Any]]:
    """Keep every command object that still has a command_id, even if the JSON is broken."""
    commands: list[dict[str, Any]] = []
    seen: set[str] = set()
    for match in _COMMAND_ID_RE.finditer(text or ""):
        command_id = match.group(1)
        if not command_id or command_id in seen:
            continue
        start = max(0, match.start() - 240)
        end = min(len(text), match.end() + 480)
        slice_ = text[start:end]
        kind_match = _KIND_RE.search(slice_)
        kind = kind_match.group(1) if kind_match else _kind_from_command_id(command_id)
        if kind is None:
            continue
        seen.add(command_id)
        commands.append({"kind": kind, "command_id": command_id, "arguments": {}})
    return commands

_PENDING_APPLY: dict[tuple[str, int, int], str] = {}


def _pending_apply_key(session_id: str, player: int, turn: int) -> tuple[str, int, int]:
    return session_id, player, turn


def remember_pending_apply(session_id: str, player: int, turn: int, payload: str) -> None:
    _PENDING_APPLY[_pending_apply_key(session_id, player, turn)] = payload


def peek_pending_apply(session_id: str, player: int, turn: int) -> str | None:
    return _PENDING_APPLY.get(_pending_apply_key(session_id, player, turn))


def publish_pending_apply(payload: str, session_id: str, player: int, turn: int) -> bool:
    """Store apply JSON, write mod pending-apply Lua, and try hidden inbox inject."""
    if not payload:
        return False
    remember_pending_apply(session_id, player, turn, payload)
    from civ5_host_channel import inject_apply_payload, write_pending_apply_lua

    write_pending_apply_lua(payload, session_id, player, turn)
    return inject_apply_payload(payload, session_id, player, turn)


def _apply_already_done_patterns(player: int, turn: int) -> tuple[str, ...]:
    # Only markers that name this turn. Lua.log tails still contain last turn's
    # inbox_apply_done / pending_apply_ok lines, which must not skip inject.
    return (
        f"apply_payload|player={player}|turn={turn}|",
        f"inbox|apply_ready|player={player}|turn={turn}|",
    )


def _apply_wait_patterns(player: int, turn: int) -> tuple[str, ...]:
    # After inject we only read new log bytes, so turn-less lines from this pulse are safe.
    return _apply_already_done_patterns(player, turn) + (
        f"bridge|inbox_apply_done|player={player}|",
        f"hostchannel|apply_pending|player={player}",
        f"bridge|apply_skip_duplicate|player={player}",
        f"bridge|pending_apply_ok|player={player}",
    )


def apply_confirmed(lua_log: Path | None, player: int, turn: int) -> bool:
    if lua_log is None or not lua_log.is_file():
        return False
    from civ5_startup_log import lua_log_tail_has_pattern

    return any(
        lua_log_tail_has_pattern(lua_log, pattern)
        for pattern in _apply_already_done_patterns(player, turn)
    )


def wait_for_apply_confirmation(
    lua_log: Path,
    player: int,
    turn: int,
    *,
    timeout_seconds: float = 30.0,
    start_offset: int | None = None,
) -> bool:
    """Wait once for any apply-success line. Do not stack per-marker timeouts."""
    if not lua_log.is_file():
        return False
    patterns = _apply_wait_patterns(player, turn)
    deadline = time.monotonic() + max(0.5, timeout_seconds)
    offset = start_offset if start_offset is not None else lua_log.stat().st_size
    while time.monotonic() < deadline:
        size = lua_log.stat().st_size
        if size < offset:
            offset = 0
        if size > offset:
            with lua_log.open("rb") as handle:
                handle.seek(offset)
                chunk = handle.read().decode("utf-8", errors="replace")
                offset = handle.tell()
            if any(pattern in chunk for pattern in patterns):
                return True
        time.sleep(0.25)
    return False


def load_chat_apply_payload(player_dir: Path) -> str | None:
    decision_path = player_dir / "decision_chat.json"
    if not decision_path.is_file():
        return None
    try:
        decision = json.loads(decision_path.read_text(encoding="utf-8-sig"))
    except (OSError, json.JSONDecodeError) as error:
        log.warning("decision_chat unreadable %s: %s", decision_path, error)
        return None
    validated = decision.get("validated") if isinstance(decision, dict) else None
    chat_messages: list[Any] = []
    if isinstance(validated, dict) and isinstance(validated.get("chat_messages"), list):
        chat_messages = validated["chat_messages"]
    if not chat_messages:
        return None
    return json.dumps({"commands": [], "chat_messages": chat_messages}, separators=(",", ":"), ensure_ascii=False)


def load_apply_payload(player_dir: Path) -> str | None:
    apply_path = resolve_apply_mailbox_path(player_dir)
    if apply_path is None:
        return None
    raw_text = ""
    try:
        raw_text = apply_path.read_text(encoding="utf-8-sig")
        apply_data = json.loads(raw_text)
    except OSError as error:
        log.warning("apply_commands unreadable %s: %s", apply_path, error)
        return None
    except json.JSONDecodeError as error:
        log.warning("apply_commands salvage %s: %s", apply_path, error)
        commands = salvage_apply_commands(raw_text)
        if not commands:
            return None
        return json.dumps({"commands": commands, "chat_messages": []}, separators=(",", ":"), ensure_ascii=False)
    commands = apply_data.get("commands") if isinstance(apply_data, dict) else None
    if not isinstance(commands, list):
        commands = salvage_apply_commands(raw_text)
    else:
        kept: list[Any] = []
        for item in commands:
            if isinstance(item, dict) and (item.get("kind") or item.get("command_id")):
                kept.append(item)
        commands = kept
    chat_messages: list[Any] = []
    if isinstance(apply_data, dict) and isinstance(apply_data.get("chat_messages"), list):
        chat_messages = apply_data["chat_messages"]
    if not chat_messages:
        decision_path = player_dir / "decision.json"
        if decision_path.is_file():
            try:
                decision = json.loads(decision_path.read_text(encoding="utf-8-sig"))
                validated = decision.get("validated") if isinstance(decision, dict) else None
                if isinstance(validated, dict) and isinstance(validated.get("chat_messages"), list):
                    chat_messages = validated["chat_messages"]
            except (OSError, json.JSONDecodeError):
                pass
    payload = {"commands": commands, "chat_messages": chat_messages}
    return json.dumps(payload, separators=(",", ":"), ensure_ascii=False)


def _player_context(player_dir: Path) -> tuple[str, int, int] | None:
    session_id = player_dir.parent.name
    if not player_dir.name.startswith("PLAYER_"):
        return None
    try:
        player = int(player_dir.name.split("_", 1)[1])
    except (IndexError, ValueError):
        return None
    turn = 0
    found_turn = False
    snapshot_path = player_dir / "snapshot.json"
    if snapshot_path.is_file():
        try:
            snapshot = json.loads(snapshot_path.read_text(encoding="utf-8-sig"))
            decision = snapshot.get("decision") if isinstance(snapshot, dict) else None
            if isinstance(decision, dict) and decision.get("turn") is not None:
                turn = int(decision["turn"])
                found_turn = True
        except (OSError, json.JSONDecodeError, TypeError, ValueError):
            found_turn = False
    if not found_turn:
        decision_path = player_dir / "decision.json"
        if decision_path.is_file():
            try:
                decision = json.loads(decision_path.read_text(encoding="utf-8-sig"))
                metrics = decision.get("metrics") if isinstance(decision, dict) else None
                if isinstance(metrics, dict) and metrics.get("turn") is not None:
                    turn = int(metrics["turn"])
            except (OSError, json.JSONDecodeError, TypeError, ValueError):
                pass
    return session_id, player, turn


def resolve_apply_mailbox_path(player_dir: Path) -> Path | None:
    from sidecar.run_civ5 import apply_mailbox_filename

    folders = [player_dir]
    if player_dir.parent.parent != player_dir:
        folders.append(player_dir.parent.parent)
    context = _player_context(player_dir)
    names: list[str] = []
    if context is not None:
        session_id, player, turn = context
        names.append(apply_mailbox_filename(session_id, player, turn))
    for name in names:
        for folder in folders:
            path = folder / name
            if path.is_file():
                return path
    return None


def publish_apply_payload(
    player_dir: Path,
    session_id: str,
    player: int,
    turn: int,
    payload: str,
) -> bool:
    lua_log: Path | None = None
    try:
        from civ5_lua_log_bridge import default_lua_log

        lua_log = default_lua_log()
    except Exception:
        pass
    marker = player_dir / f"apply_turn_{turn}.done"
    if marker.is_file():
        if apply_confirmed(lua_log, player, turn):
            log.info("Apply already confirmed for player=%s turn=%s", player, turn)
            return True
        try:
            marker.unlink()
        except OSError as error:
            log.warning("Apply marker unlink failed: %s", error)
    if lua_log is not None and lua_log.is_file() and apply_confirmed(lua_log, player, turn):
        from civ5_host_channel import clear_pending_apply_lua

        clear_pending_apply_lua()
        marker.write_text(f"applied_at={time.strftime('%Y-%m-%dT%H:%M:%S')}\n", encoding="utf-8")
        log.info("Apply already confirmed before publish player=%s turn=%s", player, turn)
        return True
    try:
        from civ5_startup_log import log_event

        civ5ai_root = player_dir.parent.parent.parent
        log_event(civ5ai_root, "inbox_apply_begin", player=player, turn=turn)
    except Exception:
        pass
    confirm_offset = lua_log.stat().st_size if lua_log is not None and lua_log.is_file() else None
    attempt_lock = player_dir / "inbox_inject_attempt.lock"
    attempt_lock.write_text(f"turn={turn}\n", encoding="utf-8")
    if not publish_pending_apply(payload, session_id, player, turn):
        return False
    wait_seconds = 60.0 if session_id.startswith("autotest-") else 20.0
    log.info("Inbox apply publish player=%s turn=%s wait=%.0fs", player, turn, wait_seconds)
    if lua_log is None or not lua_log.is_file():
        log.warning("Lua.log unavailable for apply confirmation player=%s turn=%s", player, turn)
        return False
    confirmed = wait_for_apply_confirmation(
        lua_log,
        player,
        turn,
        timeout_seconds=wait_seconds,
        start_offset=confirm_offset,
    )
    if not confirmed:
        confirmed = apply_confirmed(lua_log, player, turn)
    if not confirmed:
        log.warning("Inbox apply not confirmed in Lua.log player=%s turn=%s", player, turn)
        if session_id.startswith("autotest-"):
            try:
                from civ5_startup_log import log_event

                civ5ai_root = player_dir.parent.parent.parent
                log_event(civ5ai_root, "inbox_apply_timeout", player=player, turn=turn)
            except Exception:
                pass
        return False
    marker.write_text(f"applied_at={time.strftime('%Y-%m-%dT%H:%M:%S')}\n", encoding="utf-8")
    log.info("Inbox apply confirmed player=%s turn=%s", player, turn)
    from civ5_host_channel import clear_pending_apply_lua

    clear_pending_apply_lua()
    try:
        attempt_lock.unlink(missing_ok=True)
    except OSError:
        pass
    try:
        from civ5_startup_log import log_event

        civ5ai_root = player_dir.parent.parent.parent
        log_event(civ5ai_root, "inbox_apply_sent", player=player, turn=turn)
    except Exception:
        pass
    return True


def publish_from_player_dir(player_dir: Path, *, chat: bool = False) -> bool:
    context = _player_context(player_dir)
    if context is None:
        return False
    session_id, player, turn = context
    if chat:
        for snapshot_name in ("snapshot_chat.json", "snapshot.json"):
            snapshot_path = player_dir / snapshot_name
            if not snapshot_path.is_file():
                continue
            try:
                snapshot = json.loads(snapshot_path.read_text(encoding="utf-8-sig"))
                decision = snapshot.get("decision") if isinstance(snapshot, dict) else None
                if isinstance(decision, dict) and decision.get("turn") is not None:
                    turn = int(decision["turn"])
                    break
            except (OSError, json.JSONDecodeError, TypeError, ValueError):
                continue
        payload = load_chat_apply_payload(player_dir)
    else:
        payload = load_apply_payload(player_dir)
    if payload is None:
        return False
    return publish_apply_payload(player_dir, session_id, player, turn, payload)
