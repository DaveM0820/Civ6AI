...
def _mirror_log_dir(lua_log: Path | None) -> Path | None:
    if lua_log is None:
        return None
    return lua_log.parent / "civ6ai" / "autotest"


def startup_log_paths(civ6ai_root: Path, lua_log: Path | None = None) -> list[Path]:
    paths = [civ6ai_root / "autotest" / "startup.log"]
...
def configure_startup_logging(
    ...
    *,
    logger_name: str = "civ6ai.startup",
) -> logging.Logger:
    """Attach a file handler writing the same lines to civ6ai + Logs/civ6ai mirrors."""
    ...
    for path in startup_log_paths(civ6ai_root, lua_log):
...
def close_startup_logging(logger_name: str = "civ6ai.startup") -> None:
    logger = logging.getLogger(logger_name)
...
def log_event(
    ...
    **fields: Any,
    ...
    logger = configure_startup_logging(civ6ai_root, lua_log)
    extra = " ".join(f"{key}={value}" for key, value in sorted(fields.items()))
    message = event if not extra else f"{event} {extra}"
    logger.info(message)


def append_startup_line(civ6ai_root: Path, line: str, lua_log: Path | None = None) -> None:
    text = line.rstrip("\n") + "\n"
    for path in startup_log_paths(civ6ai_root, lua_log):
        path.parent.mkdir(parents=True, exist_ok=True)
...
def tail_lua_log_to_startup(
    ...
    *,
    ...
    if matched:
        ...
        for entry in tail:
            append_startup_line(civ6ai_root, entry, lua_log)
    return new_offset
...
def main() -> int:
    ...
    configure_startup_logging(args.civ6ai_root, args.lua_log)
    fields: dict[str, Any] = {}
    if args.session_id:
        fields["session_id"] = args.session_id
    log_event(args.civ6ai_root, args.event, lua_log=args.lua_log, **fields)
    return 0
